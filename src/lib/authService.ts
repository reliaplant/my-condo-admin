import {
  ...
  updateEmail,
  setPersistence,
  browserLocalPersistence, // Add this for persistent login
  getAuth
} from 'firebase/auth';
...
// Set persistence to LOCAL to maintain auth state across page refreshes and tabs
// This helps with deployed environments
try {
...
export const signIn = async (credentials: LoginCredentials): Promise<UserProfile> => {
  try {
    ...
    // If profile doesn't exist, create it (useful for users created in Firebase Console)
    if (!profile) {
      ...
      await setDoc(doc(db, USERS_COLLECTION, user.uid), basicProfile);
      return basicProfile;
    }
...
export const getCurrentAuthState = async (): Promise<AuthState> => {
  return new Promise((resolve) => {
    ...
    const timeoutId = setTimeout(() => {
      console.warn('Auth state initialization timed out');
      resolve({
        user: null,
        profile: null,
        initialized: true,
